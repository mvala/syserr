...
void run() {
   ...
   pt_tracking->AddGraphDirectory("/home/mvala/Temp/ALICE","pt*.txt");

   TSysError *pt_fit = new TSysError("pt_fit","Pt (fiting)");
   pt_fit->AddGraphDirectory("/home/mvala/Temp/ALICE","pt*.txt");

   TSysError *pt_norm = new TSysError("pt_norm","Pt (normalize)");
   pt_norm->AddGraphDirectory("/home/mvala/Temp/ALICE","pt*.txt");
